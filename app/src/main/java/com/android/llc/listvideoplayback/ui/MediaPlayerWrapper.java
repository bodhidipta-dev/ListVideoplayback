...
public class MediaPlayerWrapper implements MediaPlayer.OnErrorListener,
    ...
    protected MediaPlayerWrapper(MediaPlayer mediaPlayer) {
        TAG = "" + this;
        Logger.showLog(TAG, "constructor of MediaPlayerWrapper");
        Logger.showLog(TAG, "constructor of MediaPlayerWrapper, main Looper " + Looper.getMainLooper());
        Logger.showLog(TAG, "constructor of MediaPlayerWrapper, my Looper " + Looper.myLooper());
    ...
    private final Runnable mOnVideoPreparedMessage = new Runnable() {
        @Override
        public void run() {
            Logger.showLog(TAG, ">> run, onVideoPreparedMainThread");
    ...
    public void prepare() {
        ...
        Logger.showLog(TAG, ">> prepare, mState " + mState);
    ...
    private void onPrepareError(IOException ex) {
        Logger.showLog(TAG,"catch IO exception [" + ex + "]");
...
//      TODO: if (SHOW_LOGS) Logger.err(TAG, "catch exception, is Network Connected [" + Utils.isNetworkConnected());
    ...
    public void setDataSource(String filePath) throws IOException {
        synchronized (mState) {
            ...
            Logger.showLog(TAG, "setDataSource, filePath " + filePath + ", mState " + mState);
    ...
    public void onVideoSizeChanged(MediaPlayer mp, int width, int height) {
        Logger.showLog(TAG,"onVideoSizeChanged, width " + width + ", height " + height);
    ...
    public void onCompletion(MediaPlayer mp) {
        ...
        Logger.showLog(TAG,"onVideoCompletion, mState " + mState);
    ...
    public boolean onError(MediaPlayer mp, int what, int extra) {
        ...
        Logger.showLog(TAG, "onErrorMainThread, mListener " + mListener);
    ...
    private void printInfo(int what) {
        switch (what) {
            case MediaPlayer.MEDIA_INFO_UNKNOWN:
                Logger.showLog(TAG, "onInfo, MEDIA_INFO_UNKNOWN");
                break;
            case MediaPlayer.MEDIA_INFO_VIDEO_TRACK_LAGGING:
                Logger.showLog(TAG,"onInfo, MEDIA_INFO_VIDEO_TRACK_LAGGING");
                break;
            case MediaPlayer.MEDIA_INFO_VIDEO_RENDERING_START:
                Logger.showLog(TAG, "onInfo, MEDIA_INFO_VIDEO_RENDERING_START");

                break;
            case MediaPlayer.MEDIA_INFO_BUFFERING_START:

                Logger.showLog(TAG, "onInfo, MEDIA_INFO_BUFFERING_START");
                break;
            case MediaPlayer.MEDIA_INFO_BUFFERING_END:

                Logger.showLog(TAG,"onInfo, MEDIA_INFO_BUFFERING_END");
                break;
            case MediaPlayer.MEDIA_INFO_BAD_INTERLEAVING:
                Logger.showLog(TAG,"onInfo, MEDIA_INFO_BAD_INTERLEAVING");
                break;
            case MediaPlayer.MEDIA_INFO_NOT_SEEKABLE:

                Logger.showLog(TAG,"onInfo, MEDIA_INFO_NOT_SEEKABLE");
                break;
            case MediaPlayer.MEDIA_INFO_METADATA_UPDATE:
                Logger.showLog(TAG, "onInfo, MEDIA_INFO_METADATA_UPDATE");
                break;
            case MediaPlayer.MEDIA_INFO_UNSUPPORTED_SUBTITLE:
                Logger.showLog(TAG, "onInfo, MEDIA_INFO_UNSUPPORTED_SUBTITLE");

                break;
            case MediaPlayer.MEDIA_INFO_SUBTITLE_TIMED_OUT:
                Logger.showLog(TAG, "onInfo, MEDIA_INFO_SUBTITLE_TIMED_OUT");
                break;
        }
    }
    ...
    public void start() {
        ...
        synchronized (mState) {
            Logger.showLog(TAG, "start, mState " + mState);
            switch (mState.get()) {
                case IDLE:
                case INITIALIZED:
                case PREPARING:
                ...
                case PAUSED:
                    ...
                    Logger.showLog(TAG,"start, video is " + mState + ", starting playback.");
                    mMediaPlayer.start();
    ...
    public void pause() {
        ...
        synchronized (mState) {
            ...
            Logger.showLog(TAG, "pause, mState " + mState);
            switch (mState.get()) {
                case IDLE:
                case INITIALIZED:
                case PAUSED:
    ...
    private final Runnable mOnVideoStopMessage = new Runnable() {
        @Override
        public void run() {
            Logger.showLog(TAG,  ">> run, onVideoStoppedMainThread");
    ...
    public void stop() {
        Logger.showLog(TAG, ">> stop");
        synchronized (mState) {
            Logger.showLog(TAG,  "stop, mState " + mState);
            switch (mState.get()) {
    ...
    public void reset() {
        Logger.showLog(TAG, ">> reset , mState " + mState);
        synchronized (mState) {
            switch (mState.get()) {
                case IDLE:
                case INITIALIZED:
    ...
    public void release() {
        Logger.showLog(TAG, ">> release, mState " + mState);
        synchronized (mState) {
    ...
    public void clearAll() {
         Logger.showLog(TAG, ">> clearAll, mState " + mState);
        synchronized (mState) {
    ...
    public void setLooping(boolean looping) {
         Logger.showLog(TAG, "setLooping " + looping);
        mMediaPlayer.setLooping(looping);
    }

    public void setSurfaceTexture(SurfaceTexture surfaceTexture) {
         Logger.showLog(TAG, ">> setSurfaceTexture " + surfaceTexture);
         Logger.showLog(TAG, "setSurfaceTexture mSurface " + mSurface);
    ...
    public boolean isReadyForPlayback() {
        ...
        synchronized (mState) {
             Logger.showLog(TAG, "isReadyForPlayback, mState " + mState);
    ...
    public void seekToPercent(int percent) {
        synchronized (mState) {
            State state = mState.get();
             ...
             Logger.showLog(TAG, "seekToPercent, percent " + percent + ", mState " + state);
            ...
            switch (state) {
                ...
                case END:
                case STOPPED:
                    Logger.showLog(TAG,  "seekToPercent, illegal state");
                    break;
    ...
    private void startPositionUpdateNotifier() {
         Logger.showLog(TAG, "startPositionUpdateNotifier, mPositionUpdateNotifier " + mPositionUpdateNotifier);
    ...
    private void stopPositionUpdateNotifier() {
         Logger.showLog(TAG, "stopPositionUpdateNotifier, mPositionUpdateNotifier " + mPositionUpdateNotifier);
...
//             Logger.showLog(TAG, "notifyPositionUpdated, mVideoStateListener " + mVideoStateListener);
