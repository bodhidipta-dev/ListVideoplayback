...
public class VideoPlayerView extends ScalableTextureView
    ...
    public void clearPlayerInstance() {
        Logger.showLog(TAG,  ">> clearPlayerInstance");
    ...
    public void createNewPlayerInstance() {
        Logger.showLog(TAG, ">> createNewPlayerInstance");

        Logger.showLog(TAG, "createNewPlayerInstance main Looper " + Looper.getMainLooper());
        Logger.showLog(TAG, "createNewPlayerInstance my Looper " + Looper.myLooper());
        ...
        synchronized (mReadyForPlaybackIndicator){
            ...
            if(mReadyForPlaybackIndicator.isSurfaceTextureAvailable()){
                ...
                Logger.showLog(TAG, "texture " + texture);
                mMediaPlayer.setSurfaceTexture(texture);
            } else {
                Logger.showLog(TAG, "texture not available");
            }
    ...
    private void notifyOnVideoStopped() {
        Logger.showLog(TAG, "notifyOnVideoStopped");
    ...
    public void start(){
        Logger.showLog(TAG, ">> start");
        synchronized (mReadyForPlaybackIndicator){
            if(mReadyForPlaybackIndicator.isReadyForPlayback()){
                mMediaPlayer.start();
            } else {
                Logger.showLog(TAG, "start, >> wait");
                if(!mReadyForPlaybackIndicator.isFailedToPrepareUiForPlayback()){
                    ...
                    if(mReadyForPlaybackIndicator.isReadyForPlayback()){
                        ...
                        Logger.showLog(TAG, "start, movie is not ready, Player become STARTED state, but it will actually don't play");
                    }
                } else {
                    Logger.showLog(TAG, "start, movie is not ready. Video size will not become available");
                }
            }
        }
        Logger.showLog(TAG, "<< start");
    ...
    public void setDataSource(String path) {
        ...
        synchronized (mReadyForPlaybackIndicator) {
            ...
            Logger.showLog(TAG, "setDataSource, path " + path + ", this " + this);

            try {
                mMediaPlayer.setDataSource(path);

            } catch (IOException e) {
                Logger.showLog(TAG,e.getMessage());
                throw new RuntimeException(e);
            }
            mPath = path;
        }
    ...
    public void setDataSource(AssetFileDescriptor assetFileDescriptor) {
        ...
        synchronized (mReadyForPlaybackIndicator) {
            ...
            Logger.showLog(TAG,"setDataSource, assetFileDescriptor " + assetFileDescriptor + ", this " + this);
    ...
    public void onVideoSizeChangedMainThread(int width, int height) {
        ...
        Logger.showLog(TAG, ">> onVideoSizeChangedMainThread, width " + width + ", height " + height);
        ...
        if (width  != 0 && height != 0) {
            ...
            Logger.showLog(TAG, "onVideoSizeChangedMainThread, size 0. Probably will be unable to start video");
    ...
    private void notifyOnVideoSizeChangedMainThread(int width, int height) {
        Logger.showLog(TAG, "notifyOnVideoSizeChangedMainThread, width " + width + ", height " + height);
        List<MediaPlayerWrapper.MainThreadMediaPlayerListener> listCopy;
        synchronized (mMediaPlayerMainThreadListeners){
            listCopy = new ArrayList<>(mMediaPlayerMainThreadListeners);
        }
    ...
    private void notifyOnVideoCompletionMainThread() {
        Logger.showLog(TAG, "notifyVideoCompletionMainThread");
        List<MediaPlayerWrapper.MainThreadMediaPlayerListener> listCopy;
        synchronized (mMediaPlayerMainThreadListeners){
            listCopy = new ArrayList<>(mMediaPlayerMainThreadListeners);
        }
    ...
    private void notifyOnVideoPreparedMainThread() {
        Logger.showLog(TAG, "notifyOnVideoPreparedMainThread");
        List<MediaPlayerWrapper.MainThreadMediaPlayerListener> listCopy;
        synchronized (mMediaPlayerMainThreadListeners){
            listCopy = new ArrayList<>(mMediaPlayerMainThreadListeners);
        }
    ...
    private void notifyOnErrorMainThread(int what, int extra) {
        Logger.showLog(TAG, "notifyOnErrorMainThread");
        List<MediaPlayerWrapper.MainThreadMediaPlayerListener> listCopy;
        synchronized (mMediaPlayerMainThreadListeners){
            listCopy = new ArrayList<>(mMediaPlayerMainThreadListeners);
        }
    ...
    public void onErrorMainThread(final int what, final int extra) {
        Logger.showLog(TAG, "onErrorMainThread, this " + VideoPlayerView.this);
        switch (what) {
            case MediaPlayer.MEDIA_ERROR_SERVER_DIED:
                Logger.showLog(TAG, "onErrorMainThread, what MEDIA_ERROR_SERVER_DIED");
                printErrorExtra(extra);
                break;
            case MediaPlayer.MEDIA_ERROR_UNKNOWN:
                Logger.showLog(TAG, "onErrorMainThread, what MEDIA_ERROR_UNKNOWN");
                printErrorExtra(extra);
                break;
        }
    ...
    private void printErrorExtra(int extra) {
        switch (extra){
            case MediaPlayer.MEDIA_ERROR_IO:
                Logger.showLog(TAG, "error extra MEDIA_ERROR_IO");
                break;
            case MediaPlayer.MEDIA_ERROR_MALFORMED:
                Logger.showLog(TAG, "error extra MEDIA_ERROR_MALFORMED");
                break;
            case MediaPlayer.MEDIA_ERROR_UNSUPPORTED:
                Logger.showLog(TAG, "error extra MEDIA_ERROR_UNSUPPORTED");
                break;
            case MediaPlayer.MEDIA_ERROR_TIMED_OUT:
                Logger.showLog(TAG, "error extra MEDIA_ERROR_TIMED_OUT");
                break;
        }
    }
    ...
    private final Runnable mVideoSizeAvailableRunnable = new Runnable() {
        @Override
        public void run() {
            Logger.showLog(TAG, ">> run, onVideoSizeAvailable");

            synchronized (mReadyForPlaybackIndicator) {
                Logger.showLog(TAG, "onVideoSizeAvailable, mReadyForPlaybackIndicator " + mReadyForPlaybackIndicator);

                mReadyForPlaybackIndicator.setVideoSize(getContentHeight(), getContentWidth());

                if (mReadyForPlaybackIndicator.isReadyForPlayback()) {
                    Logger.showLog(TAG, "run, onVideoSizeAvailable, notifyAll");
    ...
    private void onVideoSizeAvailable() {
        Logger.showLog(TAG, ">> onVideoSizeAvailable");
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surfaceTexture, int width, int height) {
        Logger.showLog(TAG,"onSurfaceTextureAvailable, width " + width + ", height " + height + ", this " + this);
    ...
    private void notifyTextureAvailable() {
        Logger.showLog(TAG, ">> notifyTextureAvailable");

        mViewHandlerBackgroundThread.post(new Runnable() {
            @Override
            public void run() {
                Logger.showLog(TAG, ">> run notifyTextureAvailable");
                ...
                synchronized (mReadyForPlaybackIndicator) {
                    ...
                    if (mMediaPlayer != null) {
                        ...
                        mReadyForPlaybackIndicator.setVideoSize(null, null);
                        Logger.showLog(TAG, "mMediaPlayer null, cannot set surface texture");
                    }
                    mReadyForPlaybackIndicator.setSurfaceTextureAvailable(true);

                    if (mReadyForPlaybackIndicator.isReadyForPlayback()) {

                        Logger.showLog(TAG, "notify ready for playback");
                        mReadyForPlaybackIndicator.notifyAll();
                    }
                }
    ...
    public boolean onSurfaceTextureDestroyed(SurfaceTexture surface) {
        Logger.showLog(TAG, "onSurfaceTextureDestroyed, surface " + surface);
...
//        Logger.showLog(TAG, "onSurfaceTextureUpdated, mIsVideoStartedCalled " + mIsVideoStartedCalled.get() + ", mMediaPlayer.getState() " + mMediaPlayer.getState());
        if(mLocalSurfaceTextureListener != null){
    ...
    protected void onDetachedFromWindow(){
        ...
        Logger.showLog(TAG, ">> onDetachedFromWindow, isInEditMode " + isInEditMode);
        if(!isInEditMode){
            mViewHandlerBackgroundThread.postQuit();
            mViewHandlerBackgroundThread = null;
        }
        Logger.showLog(TAG, "<< onDetachedFromWindow");
    }
    ...
    protected void onVisibilityChanged(View changedView, int visibility) {
        ...
        Logger.showLog(TAG, ">> onVisibilityChanged " + visibilityStr(visibility) + ", isInEditMode " + isInEditMode);
        if (!isInEditMode) {
        ...
        Logger.showLog(TAG, "<< onVisibilityChanged");
    }
